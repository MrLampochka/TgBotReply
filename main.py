...
def init_vk():
    ...
    print(f"Successfully logged in VK as: {login} ")
...
#                     |___/                     
def dialogTurn(m):
    if str(m.chat.id) in config.module.table["vk_" + config.module.table['t_']]: return True

def key_board():
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    keyboard.add("Беседа", "Список группы", "Список преподавателей", "Расписание занятий")
    return keyboard
    ...
    for m in messages:
        if m.content_type == 'text':
            ...
            if  m.text.lower() == "в главное меню":
                module.bot.send_message(m.chat.id, 'Добро пожаловать в чат-бот группы БСС1901!', reply_markup=key_board())
            elif m.text.lower() == "беседа":
                keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)

                if dialogTurn(m):
                    keyboard.add("Выключить трансляцию")
                else:
                    keyboard.add("Включить трансляцию")
                keyboard.add("В главное меню")
                module.bot.send_message(m.chat.id,
                                        f"*Трансляция сообщений*\n\nВ данный момент трансляция {'*включена*' if dialogTurn(m) else '*выключена*'}.",
                                        reply_markup=keyboard, parse_mode="Markdown")

                keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
                keyboard.add("Беседа", "Список группы", "Список преподавателей", "Расписание занятий")

            elif m.text.lower() == "включить трансляцию":
                if not dialogTurn(m):
                    config.module.table["vk_" + str(config.module.table['t_'])].append(str(m.chat.id))
                    module.bot.send_message(m.chat.id, "*Трансляция сообщений активна!*", reply_markup=key_board(), parse_mode="Markdown")
                    open('lists/tgChatId.txt', 'a').write(str(m.chat.id)+ '\n')
                else:
                    module.bot.send_message(m.chat.id, "Трансляция уже активна!", parse_mode="Markdown")


            elif m.text.lower() == "выключить трансляцию":
                try:
                    config.module.table["vk_" + str(config.module.table['t_'])].remove(str(m.chat.id))
                    f = open('lists/tgChatId.txt', 'w')
                    for i in config.module.table["vk_" + str(config.module.table['t_'])]:
                        f.write(i + '\n')
                    module.bot.send_message(m.chat.id, "*Трансляция сообщений неактивна!*", reply_markup=key_board(), parse_mode="Markdown")

                except:
                    module.bot.send_message(m.chat.id, "Трансляция неактивна!", parse_mode="Markdown")

            elif m.text.lower() == "список преподавателей":
                module.bot.send_message(m.chat.id, codecs.open('lists/teachers.txt', "r", "utf_8_sig").read())

            elif m.text.lower() == "список группы":
                module.bot.send_message(m.chat.id, codecs.open('lists/students.txt', "r", "utf_8_sig").read())

            elif m.text.lower() == "расписание занятий":
                calKeyboard = types.InlineKeyboardMarkup(row_width=1)

                linkCal = "https://calendar.google.com/calendar/embed?src=0u5m9nrf6m0rcli0pe6n2ljl4s%40group.calendar.google.com&ctz=Europe%2FMoscow"
                linkiCal = "https://calendar.google.com/calendar/ical/0u5m9nrf6m0rcli0pe6n2ljl4s%40group.calendar.google.com/public/basic.ics"

                calKeyboard.add(types.InlineKeyboardButton("Открыть календарь в браузере", linkCal),
                                types.InlineKeyboardButton("Подписаться на календарь (iCal)", linkiCal))
                module.bot.send_photo(m.chat.id, open("lists/cal.png", 'rb'), reply_markup=calKeyboard,
...
def input_telegram():
    ...
    @module.bot.message_handler(commands=['start'])
    def start(message):

        module.bot.send_message(message.chat.id, 'Добро пожаловать в чат-бот группы БСС1901!', reply_markup=key_board())

    @module.bot.message_handler(commands=['help'])
    def start_message(message):
        module.bot.send_message(message.chat.id, 'Я умею показывать списки студентов группы и преподавателей группы '
                                                 'БСС1901, а также транслировать сообщения из VK в телеграм!')

    module.bot.infinity_polling()
